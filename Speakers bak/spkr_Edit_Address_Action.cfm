...
<cfquery name="qUpdate_Address" datasource="#application.speakerDSN#">
	Update address 
	Set speakerid='#speakerid#', add1='#Left(Form.add1, 75)#', add2='#Left(session.add2, 75)#',
		add3='#Left(session.add3, 75)#', city='#Left(Form.city, 35)#',
		state='#Left(Form.state, 4)#', zipcode='#Left(Form.zipcode, 10)#', mailtocountry='#Left(Form.mailtocountry, 35)#', busadd_1='#Left(Form.busadd_1, 75)#', busadd_2='#Left(session.busadd_2, 75)#',
		busadd_3='#Left(session.busadd_3, 75)#', buscity='#Left(Form.buscity, 35)#',
		busstate='#Left(Form.busstate, 4)#', buszip='#Left(Form.buszip, 10)#', buscountry='#Left(Form.buscountry, 35)#', timezone=#form.timezone# 
	Where add_id = #add_id# AND type = 'SPKR'
</cfquery>
