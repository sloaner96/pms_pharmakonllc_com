...
{
	window.open('mod_spkr_AddtoProject.cfm?no_menu=1&clientID=<cfoutput>#trim(form.client)#</cfoutput>&type=SPKR','','height=300,width=500,toolbar=0,statusbar=no,resizable=no,menubar=0,locationbar=0')
}
...
<cfif #form.speaker_name# NEQ '0'>
 ...
 <cfif #form.client# NEQ '0'>
	AND s.speakerid = sc.speakerid AND s.type = 'SPKR' AND sc.clientcode = '#Left(form.client, 5)#'
<cfelse>
	AND s.speakerid *= sc.speakerid AND sc.type = 'SPKR' 
</cfif> 
...
<cfif #form.city# NEQ '0' AND #form.state# NEQ '0'>
	AND s.speakerid = sa.speakerid AND s.type = 'SPKR'  AND sa.city LIKE '#form.city#%' AND sa.state LIKE '#form.state#%'
<cfelseif #form.city# NEQ '0' AND #form.state# EQ '0'>
	AND s.speakerid = sa.speakerid AND s.type = 'SPKR'  AND sa.city LIKE '#form.city#%'
<cfelseif #form.city# EQ '0' AND #form.state# NEQ '0'>
	AND s.speakerid = sa.speakerid AND s.type = 'SPKR'  AND sa.state LIKE '#form.state#%'
<cfelseif #form.city# EQ '0' AND #form.state# EQ '0'>
	AND s.speakerid = sa.speakerid AND s.type = 'SPKR'   
</cfif>	  
...
<table border="0" style="margin-left: 4px;">
	...
	<tr>
		<td width="200" align="center" valign="top">
			<form action="../Search/dsp_searchSpeaker.cfm">
				<INPUT TYPE="submit"  NAME="Search Again" VALUE="Search Again">
			</form>
		</td>
		<td width="200" align="center" valign="top">
			<form action="spkr_AddSpeaker.cfm">
...
<body> 
<font size="2">
<cfoutput><strong>#summary.recordcount#</strong></cfoutput> Records Found&nbsp;&nbsp;&nbsp;&nbsp;<a href="../Search/dsp_searchSpeaker.cfm"><u>Search Again</u></a> | <a href="dsp_AddSpeaker.cfm?add=Add+New+Speaker"><u>Add New Speaker</u></a></font><br><br>
...
<table border="0" cellspacing="0" cellpadding="2">
	...
	<tr align="left" class="highlight2">
	   ...
	   <TD height="18">&nbsp;</td>
	  	<TD><A HREF="spkr_details.cfm?speakerid=#speakerid#"><u>#lastname#&nbsp;#degree#,&nbsp;#firstname#&nbsp;#middlename#</u></a></td>
...
<table border="0" cellpadding="15px" style="margin-left: 4px;">
	<tr>
		...
		<td align="center" valign="top">
			<form action="dsp_AddSpeaker.cfm?add=Add+New+Speaker.cfm">
				<INPUT TYPE="submit"  NAME="add" VALUE="Add New Speaker">
			</form>
      	</td>
		<td align="center" valign="top">
