 ...
 ********************************************************************--->				
	<!--- pull the rows (dates) that were just inserted into availability --->
		...
		<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="Get_Times">
			SELECT year, month FROM availability_time WHERE owner_id='#id#'
		</CFQUERY>
		...
		<cfloop from="1" to="#ArrayLen(atime)#" index="x" step="1">
			<cfif atime[x][1] NEQ 100>
				<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="Insert_Times">
					INSERT availability_time(owner_id, year, month, day, start_time, end_time, allday, updated, updated_userid)		
...
<CFDEFAULTCASE>
		...
		<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="speakers_info">
			SELECT mailtocity, mailtozip, mailtostate
			FROM address
			WHERE owner_id='#id#';
		</CFQUERY>
		<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="Get_Times">
			SELECT month, year FROM availability_time WHERE owner_id='#id#' GROUP BY month, year
		</CFQUERY>
		...
		<CFOUTPUT query="speakers_info"> 
			<CFSET city="#mailtocity#">
			<CFSET state="#mailtostate#">
			<CFSET zip="#mailtozip#">		
		</CFOUTPUT>		
		<HTML>
			<HEAD>
