...
<!--- 
	*****************************************************************************************
	Name:		schedule_calendar.cfm
	Function:	Displays selected month and previous and next month for reference. 
				Each day can be one of two colors white for not available, 
				blue for available. Clicking will change the color and when submitted change 
				the database. USE ARROWS TO NAVIGATE THROUGH MONTHS!
	History:	Finalized code 8/28/01 TJS
	
	*****************************************************************************************
--->
<cfmodule template="#Application.tagpath#/ctags/header.cfm" title="Schedule Availability" showCalendar="0">
...
<SCRIPT LANGUAGE="JavaScript">
	...
	{	
		...
		{
			tempvar.style.backgroundColor = '6699FF';
			DayStatus.value = "1";
		}
		else
		{
	...
	function whichway(month, year, savemonth, saveyear, exit)
	{
		if( exit == '0')
		{
			form.action = "schedule_calendar.cfm?save=1&month=" + month + "&year=" + year + "&savemonth=" + savemonth + "&saveyear=" + saveyear;
		}
		else
		{
			form.action = "schedule_save.cfm?" + "&savemonth=" + savemonth + "&saveyear=" + saveyear;
		}
...
<tr>
	<td align="center">
	<h3><cfoutput>#SpkModName#</cfoutput></h3>
	</td>
</tr>
</table>
<TABLE ALIGN="Center" WIDTH="100%" BORDER="0" CELLPADDING="0" CELLSPACING="0" style="margin-top: 3px;">
	<TR>

<!--- Begin First Mini Calendar --->
		
		<!--- Query to find if any days are already selected --->
		<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="Previous_Month_Calendar">
			SELECT *
			FROM Availability
			WHERE month = #month_previous# AND year = #year_previous# AND ID = '#session.ID#';
		</CFQUERY>
			...
			<TABLE ALIGN="Center" BORDER="0" CELLPADDING="0" CELLSPACING="0">
				...
				<TR HEIGHT="10">
					<CFLOOP CONDITION = "DayNumbers EQ 1">
						<TD STYLE="background-color: <CFIF (#Evaluate('Previous_Month_Calendar.x#DayNumbers#')# EQ 1) AND (StartDay EQ Counter)>6699FF<CFELSE>FFFFFF</CFIF>">
					...
					<CFLOOP CONDITION = "(DayNumbers LTE DaysMonth) OR  ((DayNumbers GT DaysMonth) AND ((Counter MOD 7) NEQ 1))">
						...
						<CFIF DayNumbers LTE DaysMonth>	<!--- Create rows of boxes with the date numbers in them --->
							<CFIF (Counter MOD 7) EQ 1>	<!--- Start a new row --->
								...
								<TR HEIGHT="10">
									<TD STYLE="background-color: <CFIF #Evaluate('Previous_Month_Calendar.x#DayNumbers#')# EQ 1>6699FF<CFELSE>FFFFFF</CFIF>"><CFOUTPUT>#DayNumbers#</CFOUTPUT></TD>
...
<!--- Begin Large Calendar --->
						...
						<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="Main_Calendar">
							SELECT *
							FROM Availability
							WHERE month = #month# AND year = #year# AND ID = '#session.ID#';
						</CFQUERY>
						...
						<TABLE ALIGN="Center" WIDTH="420" BORDER="1">
							...
							<TR HEIGHT="50">
								<CFLOOP CONDITION = "DayNumbers EQ 1">
									<TD <CFIF StartDay EQ Counter>onClick="return change_status_day(this, form.Day<CFOUTPUT>#DayNumbers#</CFOUTPUT>);" onMouseOver="this.style.cursor='hand';" STYLE="background-color: <CFIF (#Evaluate("Main_Calendar.x#DayNumbers#")# EQ 1) AND (StartDay EQ Counter)>6699FF<CFELSE>FFFFFF</CFIF>"</CFIF>> 
								...
								<CFLOOP CONDITION = "(DayNumbers LTE DaysMonth) OR  ((DayNumbers GT DaysMonth) AND ((Counter MOD 7) NEQ 1))">
									...
									<CFIF DayNumbers LTE DaysMonth>	<!--- Create rows of boxes with the date numbers in them --->
										<CFIF (Counter MOD 7) EQ 1>	<!--- Start a new row --->
											...
											<TR HEIGHT="50">
												<TD onClick="change_status_day(this, form.Day<CFOUTPUT>#DayNumbers#</CFOUTPUT>);" onMouseOver="this.style.cursor='hand';" STYLE="background-color: <CFIF #Evaluate('Main_Calendar.x#DayNumbers#')# EQ 1>6699FF<CFELSE>FFFFFF</CFIF>"><INPUT TYPE="Hidden" NAME="Day<CFOUTPUT>#DayNumbers#</CFOUTPUT>" VALUE="<CFOUTPUT>#Evaluate('Main_Calendar.x#DayNumbers#')#</CFOUTPUT>"><cfoutput>#DayNumbers#</cfoutput></TD>
										<CFELSE>	<!--- Finish current row --->
												<TD onClick="change_status_day(this, form.Day<CFOUTPUT>#DayNumbers#</CFOUTPUT>);" onMouseOver="this.style.cursor='hand';" STYLE="background-color: <CFIF #Evaluate('Main_Calendar.x#DayNumbers#')# EQ 1>6699FF<CFELSE>FFFFFF</CFIF>"><INPUT TYPE="Hidden" NAME="Day<CFOUTPUT>#DayNumbers#</CFOUTPUT>" VALUE="<CFOUTPUT>#Evaluate('Main_Calendar.x#DayNumbers#')#</CFOUTPUT>"><cfoutput>#DayNumbers#</cfoutput></TD>
										</CFIF>
									<CFELSEIF (DayNumbers GT DaysMonth) AND ((Counter MOD 7) NEQ 1)>	<!--- Complete current row of boxes before quitting --->
...
<!--- Begin Second Mini Calendar --->
		<TD ALIGN="right" WIDTH="125">
			...
			<!--- Query to find if any days are already selected --->
			<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="Next_Month_Calendar">
			SELECT *
			FROM Availability
			WHERE month = #month_next# AND year = #year_next# AND ID = '#session.ID#';
			</CFQUERY>
			...
			<TABLE ALIGN="Center" BORDER="0" CELLPADDING="0" CELLSPACING="0">
				...
				<TR HEIGHT="10">
					<CFLOOP CONDITION = "DayNumbers EQ 1">
						<TD STYLE="background-color: <CFIF (#Evaluate('Next_Month_Calendar.x#DayNumbers#')# EQ 1) AND (StartDay EQ Counter)>6699FF<CFELSE>FFFFFF</CFIF>">
							<CFIF StartDay EQ Counter>
								<CFOUTPUT>#DayNumbers#</CFOUTPUT>
								<CFSET DayNumbers = 2>
							<CFELSE>
					...
					<CFLOOP CONDITION = "(DayNumbers LTE DaysMonth) OR  ((DayNumbers GT DaysMonth) AND ((Counter MOD 7) NEQ 1))">
						...
						<CFIF DayNumbers LTE DaysMonth>	<!--- Create rows of boxes with the date numbers in them --->
							<CFIF (Counter MOD 7) EQ 1>	<!--- Start a new row --->
								...
								<TR HEIGHT="10">
									<TD STYLE="background-color: <CFIF #Evaluate('Next_Month_Calendar.x#DayNumbers#')# EQ 1>6699FF<CFELSE>FFFFFF</CFIF>"><CFOUTPUT>#DayNumbers#</CFOUTPUT></TD>
							<CFELSE>	<!--- Finish current row --->
									<TD STYLE="background-color: <CFIF #Evaluate('Next_Month_Calendar.x#DayNumbers#')# EQ 1>6699FF<CFELSE>FFFFFF</CFIF>"><CFOUTPUT>#DayNumbers#</CFOUTPUT></TD>
							</CFIF>
						<CFELSEIF (DayNumbers GT DaysMonth) AND ((Counter MOD 7) NEQ 1)>	<!--- Complete current row of boxes before quitting --->
