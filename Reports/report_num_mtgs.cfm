			...
			<BODY BGCOLOR="FFFFFF" MARGINHEIGHT="0" MARGINWIDTH="0">
				...
				<TABLE ALIGN="left" WIDTH="650" BORDER="0" CELLPADDING="5" CELLSPACING="5">
					...
					<CFQUERY DATASOURCE="#application.projdsn#" NAME="meetingDT">
						...
						WHERE status = 0  
						<cfif session.id EQ 2>AND speakerid = '#session.id#'
						<cfelseif session.id EQ 3>AND speakerid = '#session.id#'
						</cfif> 
						AND (month BETWEEN #begin_month# AND #end_month#) AND (year BETWEEN #begin_year# AND #end_year#)
					</CFQUERY>
					<TR>
...
<CFDEFAULTCASE>
		...
		<HTML>
			<HEAD>
				...
				<SCRIPT>
				//create otherRMfirstname, lastname, email and voicemail if Vacant RM is chosen and the //fields do not already exist
...
spaces="    ";
data = data + "RM First Name (if vacant):" + spaces
+ "<input type='text' size=30 name=" + inter
+ "otherRMfirstname" + inter + "'><br>";


if (document.layers) {
document.layers.select_item.document.write(data);
document.layers.select_time.document.close();
}
else {
if (document.all) {
select_time.innerHTML = data;
      }
   }
}
			...
			<BODY>
				<CFOUTPUT><FORM NAME="form_select" action="report_num_mtgs.cfm?action=report" METHOD="post" onSubmit="return validate()"></CFOUTPUT>
