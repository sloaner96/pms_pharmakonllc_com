...
--->
		...
		<HTML>
			...
			<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="GetName">
				...
				FROM Speaker
				WHERE speakerid='#session.mod_id#' AND type='MOD'; 
			</CFQUERY>
			
			<cfoutput>
			<!--- Pull meetings with speaker name --->
			<CFQUERY DATASOURCE="#session.pdbs#" NAME="Getm" USERNAME="#session.pdbu#" PASSWORD="#session.pdbp#">
				SELECT m.project_code, m.start_time, m.end_time, m.year, m.month, m.day, m.meeting_date, m.speakerid, s.lastname, s.firstname
				FROM projman.dbo.ScheduleSpeaker m, speaker.dbo.Speaker s
				WHERE m.speakerid = '#session.mod_id#' AND (m.meeting_date BETWEEN #begin_date# AND #end_date#)
					AND m.speakerid *= s.speakerid AND m.status = 0
