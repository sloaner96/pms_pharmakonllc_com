...
SELECT lastname, firstname, speakerid
FROM Speaker
WHERE type = 'MOD' AND active = 'yes' 
...
SELECT lastname, firstname, speakerid
FROM Speaker
WHERE type = 'SPKR' AND active = 'yes' 
ORDER BY lastname, firstname
</CFQUERY>
...
data = data + "Select Criteria:" + spaces
+ "<SELECT name=" + inter
+ "select_criteria" + inter + "'><option value=0>(All Moderators)<cfoutput query="qmods"><option value=#qmods.speakerid#>#qmods.lastname#, #qmods.firstname#</cfoutput></select><br>";
}
...
data = data + "Select Criteria:" + spaces
+ "<SELECT name=" + inter
+ "select_criteria" + inter + "'><option value=0>(All Speakers)<cfoutput query="qspeakers"><option value=#qspeakers.speakerid#>#qspeakers.lastname#, #qspeakers.firstname#</cfoutput></select><br>";
}
