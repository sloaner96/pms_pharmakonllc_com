...
<CFQUERY DATASOURCE="#application.projdsn#" NAME="meetingDT">
	...
	m.noshow, 
	m.rowid, 
	m.staff_id AS speakerid, 
	m.client_rowid AS moderator_client, 
	m.status, 
	s.staff_id AS speakerid, 
	s.client_rowid AS speaker_client, 
	mt.firstname + ' ' + mt.lastname AS mod_name, 
	mf.fee, 
	cl.staff_id AS clisten_id, 
	cl.client_rowid AS clisten_client, 
	ml.staff_id AS mlisten_id, 
	ml.client_rowid AS mlisten_client, 
	asp.staff_id AS addspeakerid, 
	asp.client_rowid AS addspeaker_client, 
	t.staff_id AS trainee_id, 
	t.client_rowid AS trainee_client
	
	FROM 
	projman.dbo.ScheduleSpeaker m, 
	projman.dbo.ScheduleSpeaker s, 
	speaker.dbo.Speaker mt, 
	speaker.dbo.speaker_clients mf, 
	projman.dbo.ScheduleSpeaker cl, 
	projman.dbo.ScheduleSpeaker ml, 
	projman.dbo.ScheduleSpeaker asp, 
	projman.dbo.ScheduleSpeaker t
	
	WHERE (m.meeting_date BETWEEN #BeginingDate# AND #EndingDate#) AND 
	m.staff_type = 1 AND 
	m.staff_id = mt.speakerid AND 
	m.staff_id = mf.speakerid AND 
	m.status = 0 AND  
	...
	ORDER BY m.meeting_date, m.start_time, addspeakerid, mlisten_id, trainee_id, clisten_id
</CFQUERY>

<!--- <cfoutput query="meetingDT" group="meeting_code">
#meeting_code#&nbsp;&nbsp;#year#&nbsp;&nbsp;#month#&nbsp;&nbsp;#day#&nbsp;&nbsp;#start_time#&nbsp;&nbsp;#speakerid#&nbsp;&nbsp;#speakerid#<br>
</cfoutput><cfabort> --->
...
The cfc's are created always, but their methods are only called when that data it requested by user--->
	...
	<cfoutput> --->
			...
			<cfif #ReportColumns[7]# EQ 1><!---Moderator Cell--->
				<!--- <cfscript>
					GetMod = PIW.getModSpker(cfcID="#meetingDT.speakerid#");
				</cfscript> --->
			...
			<cfif #ReportColumns[9]# EQ 1><!---Speaker Cell--->
				<cfif trim(meetingDT.speakerid) EQ "">
					<cfset speaker = 1>
				<cfelse>
					<cfset speaker = #meetingDT.speakerid#>
				</cfif>
				<cfscript>
			...
			<cfif #ReportColumns[19]# EQ 1><!---Additional Speakers--->
				...
				<cfoutput group="addspeakerid">
					<cfif trim(meetingDT.addspeakerid) EQ "">
						<cfset addspeak = 1>
					<cfelse>
						<cfset addspeak = #meetingDT.addspeakerid#>
					</cfif>
				<cfscript>
