...
--->
		...
		<HTML>
			...
			<CFQUERY DATASOURCE="#application.projdsn#" NAME="Getm">
				SELECT m.project_code, m.start_time, m.end_time, m.year, m.month, m.day, m.speakerid, s.lastname, s.firstname
				FROM projman.dbo.ScheduleSpeaker m, speaker.dbo.Speaker s
				WHERE m.speakerid = '#session.id#' AND (m.month BETWEEN #startmonth# AND #endmonth#) AND (m.year BETWEEN #startyear# AND #endyear#)
