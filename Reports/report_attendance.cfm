...
<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="getm">
	...
	FROM projman.dbo.ScheduleSpeaker m, speaker.dbo.Speaker s, speaker.dbo.Speaker mo, projman.dbo.piw p, projman.dbo.recruiter r, projman.dbo.conference_company c, projman.dbo.client_proj cp
	WHERE  (m.year <= #year(now())# AND m.month <= #month(now())# AND m.day <= #year(now())#) AND m.speakerid *= s.speakerid AND m.speakerid *= mo.speakerid AND m.project_code = p.project_code AND p.recruiting_company *= r.ID AND p.conference_company *= c.ID AND m.project_code = cp.client_proj
