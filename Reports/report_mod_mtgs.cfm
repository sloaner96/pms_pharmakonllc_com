...
<TR  bgcolor="#444444">
	...
	<cfoutput query="getName">
		<CFQUERY DATASOURCE="#Application.projdsn#" NAME="getMeetingCount">
			...
			AND status = 0 
			<!--- AND speakerid = #GetName.speakerid# --->
			AND staff_id = #GetName.speakerid# AND staff_type = 1 
...
<cfoutput query="getName">
	<CFQUERY datasource="#application.projdsn#"  NAME="getMeetingCount">
		SELECT COUNT(sm.project_code) as meetCount
		FROM 
		ScheduleSpeaker ss,
		ScheduleMaster sm
		
		WHERE sm.meeting_date BETWEEN #BeginingDate# AND #EndingDate#
		AND active  = 'yes' 
		AND ss.speakerid = #GetName.speakerid# 
