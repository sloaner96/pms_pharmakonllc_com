<!--- *****************************************************************************************
	Name:		report_production_recruiter.cfm
	
	Function:	This page displays the number of meetings per week, per project based on the 
				recruiter and date range selected on report_production_select.cfm
...
<html>
<head>
<title>Recruiter Production Schedule</title>
...
<CFQUERY DATASOURCE="#application.projdsn#" NAME="getRec">
...
<CFQUERY DATASOURCE="#application.projdsn#" NAME="qrec_co">
SELECT recruiting_company, project_code 
FROM PIW
WHERE <cfif session.select_criteria NEQ 0>recruiting_company = '#session.select_criteria#'</cfif>
ORDER BY recruiting_company,  project_code
</cfquery>

<!--- Set project codes in an array and convert to list --->
<cfset RecArray = ArrayNew(1)>
<cfoutput query="qrec_co">
	<cfset RecArray[currentrow] = "'#qrec_co.project_code#'">
</cfoutput>
<cfset RecList = #ArrayToList(RecArray, ",")#>
...
<!--- pull rows from meeting summary table that match the projects pulled above --->
	...
	WHERE project_code IN (#PreserveSingleQuotes(RecList)#)
...
<cfset report_title = '#getRec.recruiter_name# Meetings Report'>
