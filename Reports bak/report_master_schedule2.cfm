...
<cfset x = 1>
	...
	<table border='1' cellpadding='4px' cellspacing='0'>
		...
		<CFQUERY DATASOURCE="#application.projdsn#" NAME="GetMeetings">
			SELECT DISTINCT m.meeting_code, m.project_code, m.start_time, m.moderator_id, m.meeting_date, m.speaker_id
			FROM schedule_meeting_time m
			WHERE m.staff_id IN (#strModeratorList#) AND (m.meeting_date BETWEEN #startdate# AND #enddate#)
			AND m.status = 0
			GROUP BY m.meeting_code, m.project_code, m.start_time, m.moderator_id, m.meeting_date, m.speaker_id 
			ORDER BY m.meeting_code, m.meeting_date, m.moderator_id					
		</CFQUERY>
		...
		<cfoutput query="GetMeetings" group="meeting_code">
			...
			<cfset meeting[currentrow][3] = #GetMeetings.moderator_id#>
						...
						<cfloop from="1" to="#GetMeetings.recordcount#" index="y" step="1">
							<cfif #meeting[y][4]# EQ #aMeetings[x][1]# AND #aModeratorArray[l]# EQ #meeting[y][3]#>
								<CFQUERY DATASOURCE="#application.speakerDSN#" NAME="GetSpeakerName">
									SELECT lastname FROM spkr_table WHERE speaker_id = #meeting[y][5]#					
								</CFQUERY>
