<!--- *****************************************************************************************
	Name:		report_production_project.cfm
	
	Function:	This page displays the number of meetings per week, per project based on the 
				project code and date range selected on report_production_select.cfm
...
<html>
<head>
<title>Project Production Schedule</title>
...
WHERE <cfif session.select_criteria NEQ 0>project_code = '#session.select_criteria#' AND</cfif> (year BETWEEN #session.begin_year# AND #session.end_year#)
...
<!--- ************************To excel********************************************************** --->
<cfset report_path = 'e:\inetpub\wwwroot\projects\cgi-bin\report_production_project2XL.htm'>
<cfset report_title_color = 'silver'>
<cfset report_col_color = 'white'>
<cfset report_columns = #num_elements#>
<cfset report_title = 'Project Meetings Report'>
...
<!--- write the headings --->
	<cfoutput>
		...
		<cffile action="append" file="#report_path#" nameconflict="overwrite" output="<td><strong>#dWeek[1][e]#</strong></td>">
		</cfloop>
