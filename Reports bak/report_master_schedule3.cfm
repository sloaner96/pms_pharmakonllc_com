...
<cfset r = 1>
	...
	<CFQUERY DATASOURCE="#application.SpeakerDSN#" NAME='getModerators'>
		SELECT speaker_id, firstname, lastname
		FROM spkr_table
		WHERE active = 'ACT'
		and type = 'MOD'
		ORDER BY speaker_id
	...
	<cfoutput query="getModerators">
		<cfset aModeratorArray[currentrow] = #getModerators.speaker_id#>
		<cfset aModeratorNameArray[currentrow] = #trim(getModerators.firstname)# & " " & #trim(getModerators.lastname)#>
			...
			SELECT allday, year, month, day, owner_id
			FROM availability_time
			WHERE owner_id = #getModerators.speaker_id#
			AND allday = 0
	...
	<CFQUERY DATASOURCE="#application.projdsn#" NAME="GetMeetings">
		SELECT m.project_code, m.meeting_code, m.start_time, m.staff_id, m.meeting_date, m.speaker_id,
				s.codeDesc as staff_description
		FROM schedule_meeting_time m, lookup s
		...
		GROUP BY m.meeting_code, m.project_code, m.start_time, m.staff_id, m.meeting_date, m.speaker_id,
		 s.CodeDesc
	...
	<cfoutput query="GetMeetings">
		...
		<!--- <cfset meeting[currentrow][5] = #GetMeetings.speaker_id#> --->
			...
			<cfset r = #r# + 1>
				...
				<cfloop from="1" to="#ArrayLen(aModeratorNameArray)#" index="l" step="1">
					...
					<cfloop from="1" to="#GetMeetings.recordcount#" index="y" step="1">
						...
						FROM speaker.dbo.spkr_table s, PMSProd.dbo.schedule_meeting_time m
						WHERE m.meeting_code = '#meeting[y][5]#' AND (staff_type = 2  OR staff_type = 5 ) AND s.speaker_id = m.staff_id
						</CFQUERY>
