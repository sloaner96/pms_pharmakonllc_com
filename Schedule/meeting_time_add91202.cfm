...
<HTML>
	...
	<!--- if there are existing meetings, check that the requested meeting does not conflict --->		
		<CFELSE>
			<CFSET rowcounter=1>
			<CFSET ROWTOTAL=check.recordcount>
			<CFSET array2d=ArrayNew(2)>
					

					<!--- set array with records scheduled meeting info --->
					<CFLOOP CONDITION="rowcounter LTE ROWTOTAL">
					<cfoutput query="check">
						<CFSET array2d[rowcounter][1] = #check.start_time#>
						<CFSET array2d[rowcounter][2] = #check.end_time#>
						<CFSET array2d[rowcounter][3] = #check.moderator_id#>
						<CFSET array2d[rowcounter][4] = #check.speaker_id#>
					</cfoutput>
			...
			<CFSET rowcounter=1>
			<CFSET found=0>
			<CFLOOP CONDITION="((rowcounter LTE ROWTOTAL) AND (found LT 1))">
			<cfoutput query="check">
				<!--- 1. If an existing meeting's begin time is equal to the requested begin time or end time...  --->
				<CFIF array2d[rowcounter][1] EQ #url.begin_time# OR array2d[rowcounter][1] EQ #url.end_time#>
				<!--- If an existing meeting's end time is equal to the requested begin time or end time...  --->
					<CFIF  array2d[rowcounter][2] EQ #url.begin_time# OR array2d[rowcounter][2] EQ #url.end_time#>
				<!--- If an existing meeting's moderator and speaker ARE NOT equal to the requested moderator and speaker...  --->
						<CFIF array2d[rowcounter][3] NEQ #url.mod_id# AND array2d[rowcounter][4] NEQ #url.spkr_id#>
				<!--- insert the meeting --->
							<!--- <CFQUERY DATASOURCE="#application.projdsn#" NAME="insert">			
								INSERT INTO schedule_meeting_time(meeting_date_id, project_code, moderator_id,speaker_id,year,month,day,start_time,end_time)
							...
							</CFQUERY> --->STOP1<cfabort>
							<CFSET found=1>
						</CFIF>		
					</CFIF>
				<!--- 2. If an existing meeting's end time is equal to the requested begin time AND end time...???? --->
				<CFELSEIF  array2d[rowcounter][2] EQ #url.begin_time# AND array2d[rowcounter][2] EQ #url.end_time#>
				<!--- If an existing meeting's moderator is equal to the requested moderator OR an existing meeting's speaker is equal to the requested speaker... --->
					<CFIF array2d[rowcounter][3] EQ #url.mod_id# OR array2d[rowcounter][4] EQ #url.spkr_id#>
		<!--- *******************************
		Added this 9/11/2002 - 
		*************************************** --->
