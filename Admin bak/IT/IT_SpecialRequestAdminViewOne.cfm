...
<CFIF IsDefined("form.job_number")>
<cfoutput><CFPARAM NAME="URL.job_number" DEFAULT="#form.job_number#"></cfoutput>
<cfelse>
<cfoutput>
<CFPARAM NAME="form.job_number" DEFAULT="#URL.job_number#">
</cfoutput>
</CFIF>
...
<HTML>
	...
	<BODY MARGINWIDTH="0" MARGINHEIGHT="0" STYLE="MARGIN: 0" BGCOLOR="WHITE">
		...
		<cfoutput>
		<TR>
			<TD CLASS="tdheader">IT View Special Request (Job Number: #form.job_number#)</TD>
		</TR>		
		<TR>
			<TD>
				...
				<TR>
					...
					<CFIF #qspec_req.comments# eq "" AND NOT IsDefined("URL.No_Menu")>
					<form method="post" action="IT_SpecialRequestAdminEdit.cfm">
					<input type="hidden" name="job_number" value="#form.job_number#">
					<input type="submit" name="submit" value=" Finish This Form ">
					</form>
					</CFIF
					></td>
					...
					<td  align="center">
					<input type="button" value=" Close Window " onClick="javascript:void window.close();">
					<br><br>
					<input type="button" value=" Print Window " onClick="javascript:void window.print();">
					</td>
					<CFELSE>
					<td  align="center">
					<form>
					<input type="button" value=" Printer Friendly Version " onClick="javascript:void window.open('IT_SpecialRequestAdminViewOne.cfm?no_menu=1&job_number=#qspec_req.job_number#','','width=650,scrollbars=yes,location=yes');">
					</form>
					</td>
					</CFIF>
